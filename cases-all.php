...
<body> 
  ...
  $sqli = "SELECT * FROM `waxiaoxia_article` WHERE id = $id and column1 = 4"; // 查询语句
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="breadcrumb">
            当前位置 : <a href="index.php">首页 </a> > <a href="cases.php">成功案例 > </a><?php echo $rowi['title'];?>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- 面包屑导航 end-->
   ...
   $sqlQ = "select * from waxiaoxia_article where id < $id and column1 = 4 order by id desc limit 0,1";
                    // 查询数据获取id的最小值
   // $Sqlmin="select id from waxiaoxia_article where id=(select min(id) from waxiaoxia_article) and column1 = 4";
   ...
   $rsQ = mysql_fetch_object($resultQ);  
                    ...
                    //下一篇文章 
   $sqlH = "select * from waxiaoxia_article where id > $id and column1 = 4 order by id asc limit 0,1"; 

                    // 查询数据获取id的最大值
   // $Sqlmax="select id from waxiaoxia_article where id=(select max(id) from waxiaoxia_article) and column1 = 4";
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="just-div">
          ...
          <?php 
          if (@$rsQ->id != '') { 
             echo "<li>上一篇：<a href='cases-all.php?id=$rsQ->id'>$rsQ->title</a></li>";
          }else{
             echo "上一篇：没有了";
          }
          if (@$rsH->id != '') {
             echo "<li>下一篇：<a href='cases-all.php?id=$rsH->id'>$rsH->title</a></li>";
          }else{
             echo "下一篇：没有了";
          }
          ?>
